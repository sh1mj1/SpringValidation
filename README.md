...
# 9.  프로젝트 준비 V4 & Form 전송 객체 분리

### 프로젝트 준비 V4

앞서 만든 기능을 유지하기 위해, 컨트롤러와 템플릿 파일을 복사합시다.

`ValidationItemControllerV4` 컨트롤러 생성 후 URL 경로도 변경하고 각 View Template 에도 적용해줍니다.
...
그렇다면 Form 전송 객체에 대해 알아보았으니 실제로 개발을 해봅시다.

### Form 전송 객체 분리 - 개발

`Item` 원복 - 이제 Item 의 검증은 사용하지 않으므로 검증 코드를 제거
...
`Item` 대신에 `ItemSaveform` 을 전달받는다. 그리고 `@Validated` 로 검증도 수행하고, `BindingResult` 로 검증 결과도 받는다.
...
실해해보면 처음 상품 등록할 때의 조건과 이후 상품 수정할 때의 조건이 각 상황에 맞게 적용되는 것을 알 수 있습니다.

### 정리

Form 전송 객체를 분리해서 등록과 수정에 딱 맞는 기능을 구성하고 검증도 명확히 분리했습니다.
